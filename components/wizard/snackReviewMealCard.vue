<template>
  <span>
    <p class="meal-nbr-index">Snacks</p>
    <div class="meal-card">
      <p class="meal-name">{{ snackName }}</p>

      <p
        v-for="snack in this.$store.state.cart.selectedSnacks"
        v-bind:key="snack.id"
        class="side-meal"
      >
        <span v-if="typeof snack.id !== 'undefined'" class="light-bold"
        >Snack item:</span
        >
        {{ snack.name }}
      </p>
...
<script>
import { getWholeSnackValues } from './../mealFunctions.js'
...
export default {
  ...
  mounted() {
    const snack = getWholeSnackValues(this.$store)
    this.calories = snack.calories
    this.fat = snack.fat
    this.protein = snack.protein
    this.carbs = snack.carbs

    this.snackName =
      this.$store.state.cart.selectedSnacks[0].name +
      ' w/ ' +
      this.$store.state.cart.selectedSnacks[1].name
  },
}
</script>
