<template>
  <div>
    <div v-if="show" class="w-meals-view-card meal-item">
      ...
      <div class="meal-info-wrapper">
        <p class="meal-item-name">
          Addons (breakfast):
          <span class="thin-text">{{ this.$store.state.cart.selectedBreakfast[0].name }} w/ {{ this.$store.state.cart.selectedBreakfast[1].name }}</span>
        </p>
        <div class="meal-values">
          <div class="value-pair">
            ...
            <p>{{ breakfastObject.calories }}g Calories</p>
          </div>
          ...
          <div class="value-pair">
            ...
            <p>{{ breakfastObject.carbs }}g Carbs</p>
          </div>
          <div class="value-spacer"></div>
          <div class="value-pair">
            <svg
            ...
            <p>{{ breakfastObject.protein }}g Protein</p>
          </div>
        </div>
      </div>
      <div class="stepper-price">
        <universal-stepper :custom-id="'breakfast-input'" :max-value="10" :event-name="'breakfast-recalc'" :defaultValue="1" />
...
import {getWholeBreakfastValues} from './../../mealFunctions.js'
import breakfastStepperVue from '../components/breakfastStepper.vue'
...
export default {
  ...
  created() {
    ...
    this.$nuxt.$on('breakfast-recalc', (stepperValue) => {
      self.getTotalPrice(stepperValue)
    })
  },
  mounted() {
    this.breakfastObject = getWholeBreakfastValues(this.$store)
    
    if (this.$store.state.cart.selectedBreakfast.length == 2)
      this.show = true
  },
  methods: {
    getTotalPrice(customMultiplication = null) {
      // get the amount of meals
      let mealPrice = this.$store.state.breakfastPrice

      // check the multiplications
      let multiplicationInput = document.getElementById('breakfast-input')
