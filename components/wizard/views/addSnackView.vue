<template>
  <div class="view">
    <div class="meal-nav">
      ...
      <p class="meal-nav-header center-abs">Add Snacks ({{ snackCount }}/2)</p>
    </div>

    <div class="view-content center-hor-margin">
      <!-- meal view -->
      <div
        v-for="snack in this.$store.state.wizard.snackMeals"
        :key="snack.id"
      ...
      >
        ...
        <div class="meal-info-wrapper">
          <p class="meal-item-name">{{ snack.name }}</p>
        ...
        <div class="buttons-wrapper">
          ...
          <stepper v-bind:dish="snack" v-bind:selected="0" />
        </div>
      </div>
      <!-- end of meal view -->
...
<script>
import stepper from './../snackStepper.vue'

export default {
  data() {
    return {
      snackCount: this.$store.state.cart.selectedSnacks.length,
    }
  },
  components: {
    stepper,
  ...
  created() {
    ...
    // assign the event
    $nuxt.$on('recalc-snack-count', () => {
      self.snackCount = this.$store.state.cart.selectedSnacks.length
    })
  },
  methods: {
    goBack() {
      $nuxt.$emit('toggle-view', 5)
      $nuxt.$emit('last-meal-selection-enter')
      $nuxt.$emit('nav-bar-toggle', true)
      $nuxt.$emit('leave-view', 5)
    },
    addToOrder() {
      $nuxt.$emit('toggle-view', 5)
      $nuxt.$emit('last-meal-selection-enter')
