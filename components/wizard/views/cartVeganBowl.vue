<template>
  <div>
    <div v-if="veganBowl" class="w-meals-view-card meal-item">
      ...
      <div class="meal-info-wrapper">
        <p class="meal-item-name">
          Addons (vegan bowl):
          <span class="thin-text">{{ veganBowl.name }}</span>
        </p>
      ...
      <div class="stepper-price">
        <universal-stepper :custom-id="'vegan-input'" :defaultValue="1" :event-name="'vegan-recalc'" :max-value="10" />
...
<script>
import { getVeganBowlInOneItem } from './../../mealFunctions.js'
...
export default {
  ...
  created() {
    ...
    this.$nuxt.$on('vegan-recalc', (stepperValue) => {
  ...
  methods: {
    getTotalPrice(customMultiplication = null) {
      ...
      let mealPrice = this.$store.state.veganBowlPrice

      // check the multiplications
      let multiplicationInput = document.getElementById('vegan-input')
