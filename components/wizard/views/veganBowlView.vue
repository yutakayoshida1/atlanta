<template>
  <div class="view">
    <div class="meal-nav">
      ...
      <p
        class="meal-nav-header center-abs"
      >Add vegan bowl ({{ veganCount }}/4)</p>
    </div>

    <!-- start of vegan bowl items -->
    <div class="view-content center-hor-margin">
      <!-- meal view -->
      <div
        v-for="veganBowl in this.$store.state.wizard.veganBowlMeals"
        :key="veganBowl.id"
      ...
      >
        ...
        <div class="meal-info-wrapper">
          <p class="meal-item-name">{{ veganBowl.name }}</p>
          <div class="meal-values">
            <div class="value-pair">
              ...
              <p>{{ veganBowl.calories }}g Calories</p>
            </div>
            ...
            <div class="value-pair">
              ...
              <p>{{ veganBowl.protein }}g Protein</p>
            </div>
          </div>
        </div>
        ...
        <div class="buttons-wrapper">
          ...
          <stepper v-bind:dish="veganBowl" v-bind:selected="0" />
        </div>
      </div>
      <!-- end of meal view -->
    </div>
    <!-- end of the meal items -->
          <!-- end of meal view -->
...
<script>
import stepper from './../components/veganBowlStepper.vue'

export default {
  data() {
    return {
      veganCount: this.$store.state.cart.selectedVeganBowl.length
    }
  },
  components: {
    stepper,
  ...
  created() {
    let self = this
    this.$nuxt.$on('recalc-vegan-bowl-count', () => {
      this.veganCount = this.$store.state.cart.selectedVeganBowl.length
    })
  },
  beforeDestroy() {
    $nuxt.$emit('leave-view', 6)
  },
  methods: {
    goBack() {
      $nuxt.$emit('toggle-view', 6)
