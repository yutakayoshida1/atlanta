<template>
  <div class="side-meal-stepper-box">
    ...
    <button
      class="stepper-btn plus-btn-breakfast"
...
export default {
  ...
  mounted() {
    ...
    for(let i = 0; i < this.$store.state.cart.selectedBreakfast.length; i++) {
      if (typeof this.$store.state.cart.selectedBreakfast[i] !== 'undefined') {
        if (this.$store.state.cart.selectedBreakfast[i].id === this.dish.id)
          this.stepperValue++
      }
    }
    ...
    if (this.$store.state.cart.selectedBreakfast.length == 2) {
      ...
      let elements = document.querySelectorAll('.plus-btn-breakfast')
  ...
  watch: {
    stepperIndex() {
      let selectedBreakfast = getBreakfastCount(this.$store, this.dish.id)
      if (this.$store.state.cart.selectedBreakfast.length == 0) {
        ...
        for (let i = 0; i < selectedBreakfast.length; i++) {
          if (typeof selectedBreakfast[i] !== 'undefined') {
            this.stepperValue++
          }
        }
      }

      if (selectedBreakfast == 2) {
        this.increaseAvailable = false
        let elements = document.querySelectorAll('.plus-btn-breakfast')
        for (let i = 0; i < elements.length; i++) {
          elements[i].setAttribute('disabled', 'true')
  ...
  methods: {
    ...
    addSelectedDishFromStepper() {
      if (this.$store.state.cart.selectedBreakfast.length < 2) {
        ...
        values.index = this.$store.state.cart.selectedBreakfast.length
        values.data = this.dish
        // add the selected breakfast
        this.$store.commit('cart/setBreakfastItem', values)
        ...
        $nuxt.$emit('show-and-count-points', true, this.$store.state.breakfastPrice)
        $nuxt.$emit('check-addons')
        this.$store.commit('wizard/refreshPrice', this.$store)
        $nuxt.$emit('breakfast-values-refresh')
      }
    },
    removeSelectedDishFromStepper() {
      if (this.$store.state.cart.selectedBreakfast.length > 0) {
        // remove breakfast
        this.$store.commit('cart/removeBreakfast', this.dish.id)
        this.stepperValue--
