<template>
  <div class="side-meal-stepper-box">
    ...
    <button
      class="stepper-btn plus-btn-snack"
...
export default {
  ...
  mounted() {
    ...
    for(let i = 0; i < this.$store.state.cart.selectedSnacks.length; i++) {
      if (typeof this.$store.state.cart.selectedSnacks[i] !== 'undefined') {
        if (this.$store.state.cart.selectedSnacks[i].id === this.dish.id)
          this.stepperValue++
      }
    }
    ...
    if (this.$store.state.cart.selectedSnacks.length == 2) {
      ...
      let elements = document.querySelectorAll('.plus-btn-snack')
  ...
  watch: {
    stepperIndex() {
      let selectedSnacks = getBreakfastCount(this.$store, this.dish.id)
      if (this.$store.state.cart.selectedSnacks.length == 0) {
        ...
        for (let i = 0; i < selectedSnacks.length; i++) {
          if (typeof selectedSnacks[i] !== 'undefined') {
            this.stepperValue++
          }
        }
      }
  ...
  methods: {
    ...
    addSelectedDishFromStepper() {
      if (this.$store.state.cart.selectedSnacks.length < 2) {
        ...
        values.index = this.$store.state.cart.selectedSnacks.length
        values.data = this.dish
        // add the selected breakfast
        this.$store.commit('cart/setSnackItem', values)
        ...
        $nuxt.$emit('show-and-count-points', true, this.$store.state.snackPrice)
        $nuxt.$emit('check-addons')
        $nuxt.$emit('recalc-snack-count')
        this.$store.commit('wizard/refreshPrice', this.$store)
        $nuxt.$emit('snack-values-refresh')
      }
    },
    removeSelectedDishFromStepper() {
      if (this.$store.state.cart.selectedSnacks.length > 0) {
        // remove breakfast
        this.$store.commit('cart/removeSnack', this.dish.id)
        this.stepperValue--
