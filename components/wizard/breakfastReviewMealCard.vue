<template>
  <span>
    <p class="meal-nbr-index">Breakfast</p>
    <div class="meal-card">
      <p class="meal-name">{{ breakfastName }}</p>

      <p
        v-for="breakfast in this.$store.state.cart.selectedBreakfast"
        v-bind:key="breakfast.id"
        class="side-meal"
      >
        <span v-if="typeof breakfast.id !== 'undefined'" class="light-bold"
          >Breakfast item:</span
        >
        {{ breakfast.name }}
      </p>
...
export default {
  ...
  mounted() {
    const breakfast = getWholeBreakfastValues(this.$store)
    this.calories = breakfast.calories
    this.fat = breakfast.fat
    this.protein = breakfast.protein
    this.carbs = breakfast.carbs

    this.breakfastName =
      this.$store.state.cart.selectedBreakfast[0].name +
      ' w/ ' +
      this.$store.state.cart.selectedBreakfast[1].name
  },
}
</script>
