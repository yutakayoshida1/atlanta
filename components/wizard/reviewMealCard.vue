<template>
  <span v-if="meal">
    <p class="meal-nbr-index">Meal {{ mealIndex }}</p>
    <div class="meal-card">
      <p v-if="meal" class="meal-name">{{ meal.name }}</p>

      <p v-for="dish in dishes" v-bind:key="dish.id" class="side-meal">
        <span v-if="typeof dish.id !== 'undefined'" class="light-bold">Side:</span> {{ dish.name }}
      </p>
...
<script>
import { getProductByIndex, getAllMealsByIndex } from './../mealFunctions.js'
...
export default {
  ...
  mounted() {
    // take care of the assigning values
    if (typeof this.mealIndex !== 'undefined') {
      // get the meal item by its index
      this.meal = getProductByIndex(this.$store, this.mealIndex)
      this.dishes = getAllMealsByIndex(this.$store, this.mealIndex - 1)

      if (typeof this.meal !== 'undefined') {
        // sum the meal values
        this.totalCalories = this.meal.calories
        this.totalFat = this.meal.fat
        this.totalCarbs = this.meal.carbs
        this.totalProtein = this.meal.protein
      }
      ...
      if (typeof this.dishes !== 'undefined') {
        ...
        for(let i = 0; i < this.dishes.length; i++) {
          if (typeof this.dishes[i] !== 'undefined') {
            console.log('test02')
            this.totalCalories = +this.totalCalories + +this.dishes[i].calories
            this.totalFat = +this.totalFat + +this.dishes[i].fat
            this.totalCarbs = +this.totalCarbs + +this.dishes[i].carbs
            this.totalProtein = +this.totalProtein + +this.dishes[i].protein
          }
        }
      }
    }
