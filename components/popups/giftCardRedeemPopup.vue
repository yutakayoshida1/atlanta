  ...
  <div class="popup-bg">
    <div class="popup-wizard">
      <div class="popup-wizard-content">
        ...
        <h2 class="delete-header">Redeem a Gift Card</h2>

        <div class="inside-content-wrapper">
          <p :class="{ 'error-class': inputError }" class="bold-p">{{ paragraph }}</p>
          <div class="ext-input-wrp duplicate-input">
            <input v-model="giftCard" type="text" value="" />
          </div>
        </div>
...
export default {
  data() {
    return {
      giftCard: '',
      paragraph: 'Enter your gift card code',
      inputError: false
    }
  },
  mounted() {
  ...
  methods: {
    close() {
      $nuxt.$emit('close-redeem-card')
    },
    async submit() {
      // make an API call to the backend
      const activation = await callUrlWithJwt(
        this.$axios,
        this.$store,
        this.$store.state.apiConfiguration.baseUrl +
          this.$store.state.apiConfiguration.urlActivateGiftCard,
        'POST',
      ...
      if (activation.status === 'success') {
        ...
        // show error message and turn input to red
        this.paragraph = activation.message
