  ...
  <div class="popup-bg">
    <div class="popup-wizard">
      <div class="popup-wizard-content">
        ...
        <h2 class="delete-header">Share via Email</h2>
        <p class="email-info">Email will be sent from</br>atlantaprepmeal@gmail.com</p>

        <div class="inside-content-wrapper">
          <p class="input-label">To:</p>
          <div class="ext-input-wrp duplicate-input">
            <input v-model="to" type="email" placeholder="email@gmail.com"  />
          </div>

          <p style="margin-top: 24px;" class="input-label">Subject:</p>
          <div class="ext-input-wrp duplicate-input">
        ...
        <div class="button-row">
          ...
          <button @click="submit()" class="ms-button-wide ms-filled-button meal-button">
            Submit
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    close() {
      $nuxt.$emit('close-share-email')
    },
    async submit() {
      // make an API call to the backend
      if (this.message !== '' && this.to !== '' && this.subject !== '') {
        // call the API
        const emailInvite = await callUrlWithJwt(
          this.$axios,
          this.$store,
          this.$store.state.apiConfiguration.baseUrl +
          this.$store.state.apiConfiguration.urlEmailInvite,
          'POST',
          {
        ...
        if (emailInvite.status === 'success') {
          this.close()
        }
      }else {
        // log error
        console.error('Not all inputs were filled')
      }
    },
  },
}
