<template>
  <div class="popup-bg">
    <div class="popup-wizard">
      <img v-if="meal" class="product-img" :src="meal.image" alt="" />
      <div class="popup-wizard-content">
        ...
        <div v-if="meal" class="grey-tag-box">
          <strong>Contains:</strong> {{ meal.contains }}
        </div>
        <span class="mobile-popup-btns">
          ...
          <button class="ms-button-wide ms-filled-button meal-button" @click="selectMeal()">
            ADD
          </button>
        </span>
      </div>
...
<script>
import { getProductById } from './../mealFunctions.js'
...
export default {
  ...
  mounted() {
    ...
    this.meal = getProductById(this.$store, this.mealId)

    if(window.location.hostname === 'localhost')
      this.domain = window.location.protocol + '//' + window.location.hostname + ':8888'
    else
  ...
  methods: {
    ...
    selectMeal() {
      $nuxt.$emit('select-main-meal', this.meal)
      this.close()
    }
  },
}
